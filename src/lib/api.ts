...
export const toiletAPI = {
  ...
  // 화장실 삭제하기
  delete: (id: string) => apiRequest(`/toilets/${id}`, {
    method: 'DELETE',
  }),
};
...
export const ratingAPI = {
  ...
  getByUser: (userId: string) =>
    apiRequest(`/ratings/users/${userId}/ratings`),
...
export const editRequestAPI = {
  // 수정 요청 제출하기
  create: (toiletId: string, userId: string, reason: string, description?: string) =>
  ...
  getByToilet: (toiletId: string, status?: string) =>
    apiRequest(`/edit-requests/toilets/${toiletId}/edit-requests${status ? `?status=${status}` : ''}`),

  // 사용자별 수정 요청 목록 가져오기
  getByUser: (userId: string, status?: string) =>
    apiRequest(`/edit-requests/users/${userId}/edit-requests${status ? `?status=${status}` : ''}`),

  // 관리자: 모든 수정 요청 조회
  getAllAdmin: (status?: string, page: number = 1, limit: number = 20) =>
    apiRequest(`/edit-requests/admin/edit-requests?${new URLSearchParams({
      ...(status && { status }),
      page: page.toString(),
  ...
  // 관리자: 수정 요청 승인/거부
  updateStatus: (requestId: string, status: 'approved' | 'rejected', adminResponse?: string) =>
